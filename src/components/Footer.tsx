...
const footerLinks = {
  ...
  Soluciones: [
    { label: "Aulas Hibridas", href: "/soluciones" },
...
export default function Footer() {
  return (
    <>
      {/* ── CTA Section - Dark contrast section ────────────────── */}
          ...
          <div className="flex flex-col lg:flex-row items-center justify-between gap-8">
            <div className="text-center lg:text-left">
              ...
              <p className="text-slate-400">
                Contactanos hoy mismo, seguinos en redes o escribinos por WhatsApp.
              </p>
            </div>
            ...
            <div className="flex flex-wrap items-center justify-center gap-3">
              ...
              <Link 
                href="https://instagram.com/laclasedigital" 
                target="_blank" 
                rel="noopener noreferrer" 
                className="flex items-center gap-2 px-5 py-3 rounded-xl text-sm font-semibold text-white border border-white/20 hover:bg-white/10 transition-all"
              >
        ...
        <div className="max-w-7xl mx-auto px-6 py-16">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-12">
            ...
            <div className="lg:col-span-2">
              <Link href="/" className="inline-block mb-6">
                <span className="text-xl font-bold text-[var(--foreground)]">
                  La Clase{" "}
                  <span className="text-[var(--brand-emerald)]">Digital</span>
                </span>
              </Link>
              <p className="text-[var(--foreground-muted)] text-sm leading-relaxed mb-8 max-w-xs">
                Mas de 15 anos transformando la educacion con tecnologia de vanguardia en Argentina, Paraguay y Uruguay.
              </p>
              ...
              <div className="space-y-4">
                {[
                  ...
                  <div key={text} className="flex items-center gap-3 text-sm text-[var(--foreground-muted)]">
                    <Icon className="w-4 h-4 shrink-0 text-[var(--brand-emerald)]" />
                    {text}
                  </div>
                ))}
              </div>
            ...
            {Object.entries(footerLinks).map(([title, links]) => (
              <div key={title}>
                <h3 className="text-sm font-semibold text-[var(--foreground)] mb-5">{title}</h3>
                <ul className="space-y-3">
                  {links.map((link) => (
                    <li key={link.label}>
                      <Link
                        href={link.href}
                        className="text-sm text-[var(--foreground-muted)] hover:text-[var(--brand-emerald)] transition-colors"
                      >
                        {link.label}
                      </Link>
                    </li>
            ...
            <div className="flex items-center gap-1">
              {socials.map(({ Icon, href, label }) => (
                <Link
                  ...
                  rel="noopener noreferrer"
                  aria-label={label}
                  className="w-10 h-10 flex items-center justify-center rounded-lg text-[var(--foreground-muted)] hover:text-[var(--brand-emerald)] hover:bg-[var(--brand-emerald-glow)] transition-all"
                >
